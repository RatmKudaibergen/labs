...
def not_dots(): 
    if ACTIVE_TOOL != 'circle' and current_figure != None: return current_figure.w > 1 and current_figure.h > 1
...
def accident_draw():
    ...
    if current_figure != None and not_dots(): 
...
def bfs_fill(origin_color, fill_color, x, y):
    ...
    def step(x, y):
        ...
        if x >= surf_for_draw[0].w or y >= surf_for_draw[0].h: return False
        if surf_for_draw[0].surface.get_at((x, y)) != origin_color: return False
        queue.append((x, y))
        surf_for_draw[0].surface.set_at((x, y), fill_color)
    while len(queue) > 0:
        start = queue[0]
...
while run:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        if event.type == pygame.KEYDOWN : # NOT NECESSARY
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button == 1 and surf_for_draw[1].rect.collidepoint(event.pos) and stage_of_figure == 2:
                ...
                if not any([i for i in change_figure_bottom.values()]):
                    ...
                    if current_figure != None: #points
                        stage_of_figure = 3
                        if ACTIVE_TOOL == 'rectangle' or ACTIVE_TOOL == 'r-triangle' or ACTIVE_TOOL == 'romb':
            ...
            if event.button == 1 and surf_for_draw[1].rect.collidepoint(event.pos) and stage_of_figure%5 == 0 and not (surf_change_by_diagonal or surf_change_by_right or surf_change_by_bottom or CHANGE_COLOR):
                ...
                if ACTIVE_TOOL == 'rectangle' or ACTIVE_TOOL == 'square' or ACTIVE_TOOL == 'r-triangle' or ACTIVE_TOOL == 'romb': 
                    current_figure = figure.FIGURE(ACTIVE_TOOL, 1, 1, start_x, start_y, 0, ACTIVE_SIZE)
                elif ACTIVE_TOOL == 'e-triangle':
                    current_figure = figure.FIGURE(ACTIVE_TOOL, 1, 2, start_x, start_y, 0, ACTIVE_SIZE)
                elif ACTIVE_TOOL == 'circle':
                    current_figure = figure.FIGURE(ACTIVE_TOOL, 0, 0, start_x, start_y, 1, ACTIVE_SIZE)
            if event.button == 1 and ACTIVE_TOOL!='r-triangle' and ACTIVE_TOOL!='e-triangle' and ACTIVE_TOOL!='romb' and main_size_picker.rect.collidepoint(event.pos) and size_picker_stage!=2:
                size_picker_stage = 2
            ...
            if stage_of_figure == 5 and not size_picker_stage==2 and not CHANGE_COLOR:
                ...
                stage_of_figure = 0
            #Fill:
            if event.button == 1 and ACTIVE_TOOL == 'fill':
                if surf_for_draw[0].rect.collidepoint(event.pos) and not (surf_change_by_diagonal or surf_change_by_right or surf_change_by_bottom):
                    points = figure.from_win_to_working_surf_position(event, h)
            ...
            # Accident draw figure:
            if event.button == 1 and not surf_for_draw[1].rect.collidepoint(event.pos) and not (CHANGE_COLOR or size_picker_stage == 2):
                if current_figure != None: accident_draw()
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if surf_change_by_diagonal:
                ...
                if ACTIVE_TOOL == 'rectangle' or ACTIVE_TOOL == 'r-triangle' or ACTIVE_TOOL == 'romb':
                    ...
                    current_figure.endx = current_figure.x + current_figure.r
                    current_figure.endy = current_figure.y + current_figure.r
                elif ACTIVE_TOOL == 'square' or ACTIVE_TOOL == 'e-triangle':
                    current_figure.x = stage_4['start_coord_figure'][0] + dx
                    current_figure.y = stage_4['start_coord_figure'][1] + dy
                    current_figure.startx = current_figure.x
                    current_figure.starty = current_figure.y
                    if ACTIVE_TOOL == 'square':
                        current_figure.endx = current_figure.x + min(current_figure.w, current_figure.h)
                        current_figure.endy = current_figure.y + min(current_figure.w, current_figure.h)
                    elif ACTIVE_TOOL == 'e-triangle':
                        current_figure.endx = current_figure.x + current_figure.w
                        current_figure.endy = current_figure.y + current_figure.h
                current_figure.update_surf_rect()
            # SIZE PICKER:
